...
- [x] Rebuild the app's existing screens against the element catalogue (§4) — as far as that's meaningful without building new features. Full item-by-item audit now lives in **`notes/design-fidelity-checklist.md`** (first pass 2026-09-09) rather than duplicated here; summary: masthead ✅ (added the ember mono note-count kicker, §4.1's "scale before identity" ordering), search field ✅, **snippet card ✅** (real gap closed — tag chips, a backlink-count chip, and a mono timestamp added to the note list, backed by a `snippet`/`backlinkCount` addition to `NoteSummary` that reuses the index's already-derived backlink graph, zero new disk I/O), wikilink autocomplete ✅ (restyled the CodeMirror popup itself, verified via computed style — not just the source), search results ✅ (match highlighting via real `<mark>`, plus the same relationship-chip/timestamp treatment as the snippet card). Sort button, section rail/view-mode switcher, folder/file row, docked bar, format pop menu, tag-row expander stay ❌/not applicable — each needs a feature (sort, folders, format toolbar, tag editing) this pass didn't build, tracked in the checklist file with why.
