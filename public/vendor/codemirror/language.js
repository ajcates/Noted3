/* esm.sh - @codemirror/language@6.12.4 */
import {
  IterMode as mt,
  NodeProp as y,
  NodeSet as oe,
...
import {
  highlightTree as ce,
  styleTags as de,
  tagHighlighter as ue,
  tags as u,
} from "@lezer/highlight";
import { StyleModule as gt } from "style-mod";
var X, S = new y();
function Ft(n) {
  return v.define({ combine: n ? (t) => t.concat(n) : void 0 });
}
var pe = new y(),
  d = class {
    constructor(t, e, r = [], i = "") {
      this.data = t,
        this.name = i,
        D.prototype.hasOwnProperty("tree") ||
        Object.defineProperty(D.prototype, "tree", {
          get() {
            return g(this);
          },
        }),
        ...
        this.extension = [
          x.of(this),
          D.languageData.of((s, o, a) => {
            let l = kt(s, o, a), h = l.type.prop(S);
            if (!h) return [];
            let f = s.facet(h), c = l.type.prop(pe);
            if (c) {
              let w = l.resolve(o - l.from, a);
              for (let k of c) {
                if (k.test(w, s)) {
                  let P = s.facet(k.facet);
    ...
    isActiveAt(t, e, r = -1) {
      return kt(t, e, r).type.prop(S) == this.data;
    }
    findRegions(t) {
      let e = t.facet(x);
      if (e?.data == this.data) return [{ from: 0, to: t.doc.length }];
      if (!e || !e.allowsNesting) return [];
      let r = [],
        i = (s, o) => {
          if (s.prop(S) == this.data) {
...
d.setState = K.define();
function kt(n, t, e) {
  let r = n.facet(x), i = g(n).topNode;
  if (!r || r.allowsNesting) {
    for (
      let s = i;
      s;
      s = s.enter(t, e, mt.ExcludeBuffers | mt.EnterBracketed)
    ) s.type.isTop && (i = s);
  }
  return i;
}
var bt = class n extends d {
  constructor(t, e, r) {
    super(t, e, [], r), this.parser = e;
  }
  static define(t) {
    let e = Ft(t.languageData);
    return new n(
      e,
      t.parser.configure({ props: [S.add((r) => r.isTop ? e : void 0)] }),
      t.name,
    );
  }
  configure(t, e) {
    return new n(this.data, this.parser.configure(t), e || this.name);
  }
  get allowsNesting() {
...
function me(n, t, e = 50) {
  var r;
  let i = (r = n.field(d.state, !1)) === null || r === void 0
    ? void 0
    : r.context;
  if (!i) return null;
  let s = i.viewport;
...
function hn(n, t = n.doc.length) {
  var e;
  return ((e = n.field(d.state, !1)) === null || e === void 0
    ? void 0
    : e.context.isDone(t)) || !1;
}
function fn(n, t = n.viewport.to, e = 100) {
  let r = me(n.state, t, e);
  return r != g(n.state) && n.dispatch({}), !!r;
}
function un(n) {
  var t;
  return ((t = n.plugin(Wt)) === null || t === void 0
    ? void 0
    : t.isWorking()) || !1;
}
var tt = class {
    ...
    read(t, e) {
      ...
      return t < r || e >= this.cursorPos
        ? this.doc.sliceString(t, e)
        : this.string.slice(t - r, e - r);
    }
  },
  I = null,
  B = class n {
    ...
    static create(t, e, r) {
      return new n(t, e, [], m.empty, 0, r, [], null);
    }
    startParse() {
      return this.parser.startParse(new tt(this.state.doc), this.fragments);
    }
    work(t, e) {
      return e != null && e >= this.state.doc.length && (e = void 0),
        this.tree != m.empty && this.isDone(e ?? this.state.doc.length)
          ? (this.takeTree(), !0)
          : this.withContext(() => {
            var r;
            if (typeof t == "number") {
            ...
            for (
              this.parse || (this.parse = this.startParse()),
                ...
                e < this.state.doc.length && this.parse.stopAt(e);;
            ) {
              let i = this.parse.advance();
              if (i) {
                if (
                  this.fragments = this.withoutTempSkipped(
                    H.addTree(i, this.fragments, this.parse.stoppedAt != null),
                  ),
                    this.treeLen =
    ...
    takeTree() {
      let t, e;
      this.parse && (t = this.parse.parsedPos) >= this.treeLen &&
        ((this.parse.stoppedAt == null || this.parse.stoppedAt > t) &&
          ...
          this.tree = e,
          this.fragments = this.withoutTempSkipped(
            H.addTree(this.tree, this.fragments, !0),
          ),
          this.parse = null);
    }
    withContext(t) {
    ...
    withoutTempSkipped(t) {
      for (let e; e = this.tempSkipped.pop();) t = wt(t, e.from, e.to);
      return t;
    }
    changes(t, e) {
      ...
      if (this.takeTree(), !t.empty) {
        let l = [];
        if (
          t.iterChangedRanges((h, f, c, w) =>
            l.push({ fromA: h, toA: f, fromB: c, toB: w })
          ),
            r = H.applyChanges(r, l),
            i = m.empty,
            s = 0,
      ...
      return new n(this.parser, e, r, i, s, o, a, this.scheduleOn);
    }
    updateViewport(t) {
      ...
      for (let r = 0; r < this.skipped.length; r++) {
        ...
        i < t.to && s > t.from &&
          (this.fragments = wt(this.fragments, i, s),
        ...
        createParse(e, r, i) {
          ...
          return {
            ...
            advance() {
              let l = I;
              if (l) {
                for (let h of i) l.tempSkipped.push(h);
                t &&
                  (l.scheduleOn = l.scheduleOn
                    ? Promise.all([l.scheduleOn, t])
                    : t);
              }
              return this.parsedPos = o, new m(_.none, [], [], o - s);
            },
            stoppedAt: null,
            stopAt() {},
          };
...
var M = class n {
  ...
  apply(t) {
    ...
    let e = this.context.changes(t.changes, t.state),
      r = this.context.treeLen == t.startState.doc.length
        ? void 0
        : Math.max(t.changes.mapPos(this.context.treeLen), e.viewport.to);
    return e.work(20, r) || e.takeTree(), new n(e);
  }
  static init(t) {
    let e = Math.min(3e3, t.doc.length),
      r = B.create(t.facet(x).parser, t, { from: 0, to: e });
    return r.work(20, e) || r.takeTree(), new n(r);
  }
};
d.state = Mt.define({
  create: M.init,
  update(n, t) {
    for (let e of t.effects) if (e.is(d.setState)) return e.value;
    return t.startState.facet(x) != t.state.facet(x)
      ? M.init(t.state)
      : n.apply(t);
  },
});
...
typeof requestIdleCallback < "u" && (Ut = (n) => {
  let t = -1,
    e = setTimeout(() => {
      t = requestIdleCallback(n, { timeout: 400 });
    }, 100);
  return () => t < 0 ? clearTimeout(e) : cancelIdleCallback(t);
});
var Y = typeof navigator < "u" &&
      (!((X = navigator.scheduling) === null || X === void 0) &&
        X.isInputPending)
    ...
    class {
      ...
      update(t) {
        ...
        (e.updateViewport(t.view.viewport) ||
          this.view.viewport.to > e.treeLen) && this.scheduleWork(),
      ...
      scheduleWork() {
        ...
        (e.tree != e.context.tree || !e.context.isDone(t.doc.length)) &&
          (this.working = Ut(this.work));
      }
      work(t) {
        this.working = null;
        let e = Date.now();
        if (
          this.chunkEnd < e && (this.chunkEnd < 0 || this.view.hasFocus) &&
        ...
        let o = Date.now() +
            Math.min(
              this.chunkBudget,
              100,
              t && !Y ? Math.max(25, t.timeRemaining() - 5) : 1e9,
            ),
        ...
        this.chunkBudget -= Date.now() - e,
          ...
          (s.context.takeTree(),
            this.view.dispatch({ effects: d.setState.of(new M(s.context)) })),
      ...
      checkAsyncSchedule(t) {
        t.scheduleOn &&
          (this.workScheduled++,
            t.scheduleOn.then(() => this.scheduleWork()).catch((e) =>
              ae(this.view.state, e)
  ...
  vt = class n {
    ...
    load() {
      return this.loading ||
        (this.loading = this.loadFunc().then((t) => this.support = t, (t) => {
    ...
    static of(t) {
      ...
      return new n(
        t.name,
        (t.alias || []).concat(t.name).map((i) => i.toLowerCase()),
        t.extensions || [],
        t.filename,
        e,
        r,
    ...
    static matchFilename(t, e) {
      ...
      let r = /\.([^.]+)$/.exec(e);
      if (r) { for (let i of t) if (i.extensions.indexOf(r[1]) > -1) return i; }
      return null;
    }
    static matchLanguageName(t, e, r = !0) {
      e = e.toLowerCase();
      for (let i of t) if (i.alias.some((s) => s == e)) return i;
      if (r) {
        for (let i of t) {
          for (let s of i.alias) {
            ...
            if (
              o > -1 &&
              (s.length > 2 ||
                !/\w/.test(e[o - 1]) && !/\w/.test(e[o + s.length]))
            ) return i;
          }
        }
      }
...
function Vt(n, t) {
  let e = "", r = n.tabSize, i = n.facet(Ht)[0];
  if (i == "	") {
    for (; t >= r;) e += "	", t -= r;
    i = " ";
  }
  for (let s = 0; s < t; s++) e += i;
  return e;
}
function $t(n, t) {
  n instanceof D && (n = new N(n));
  for (let r of n.state.facet(ge)) {
    let i = r(n, t);
    if (i !== void 0) return i;
  }
  let e = g(n.state);
  return e.length >= t ? ke(n, e, t) : null;
}
function dn(n, t, e) {
  ...
  for (let o = t; o <= e;) {
    ...
    if (l == null) continue;
    /\S/.test(a.text) || (l = 0);
    let h = /^\s*/.exec(a.text)[0], f = Vt(n, l);
    h != f &&
      (r[a.from] = l,
        s.push({ from: a.from, to: a.from + h.length, insert: f }));
  }
  return n.changes(s);
}
var N = class {
    constructor(t, e = {}) {
      this.state = t, this.options = e, this.unit = j(t);
    }
    lineAt(t, e = 1) {
      let r = this.state.doc.lineAt(t),
    ...
    countColumn(t, e = t.length) {
      return le(t, this.state.tabSize, e);
    }
    lineIndent(t, e = 1) {
      let { text: r, from: i } = this.lineAt(t, e),
  ...
  jt = new y();
function ke(n, t, e) {
  let r = t.resolveStack(e),
    i = t.resolveInner(e, -1).resolve(e, 0).enterUnfinishedNodesBefore(e);
  if (i != r.node) {
    let s = [];
    for (
      let o = i;
      o &&
      !(o.from < r.node.from || o.to > r.node.to ||
        o.from == r.node.from && o.type == r.node.type);
      o = o.parent
    ) s.push(o);
...
var et = class n extends N {
  ...
  continue() {
    return zt(this.context.next, this.base, this.pos);
  }
};
function ve(n, t) {
...
function pn({ closing: n, align: t = !0, units: e = 1 }) {
  return (r) => qt(r, t, e, n);
}
function qt(n, t, e, r, i) {
  let s = n.textAfter,
    ...
    l = t ? xe(n) : null;
  return l
    ? a ? n.column(l.from) : n.column(l.to)
    : n.baseIndent + (a ? 0 : n.unit * e);
}
var mn = (n) => n.baseIndent;
...
function Ae(n, t, e) {
  ...
  for (let o = i; o; o = o.next) {
    let a = o.node;
    if (a.to <= e || a.from > e) continue;
    if (s && a.from < t) break;
    let l = a.type.prop(Pe);
    if (l && (a.to < r.length - 50 || r.length == n.doc.length || !Ce(a))) {
      let h = l(a, n);
...
var U = K.define({ map: Gt }), C = K.define({ map: Gt });
function ft(n) {
  let t = [];
...
var T = Mt.define({
  ...
  update(n, t) {
    ...
    t.changes.iterChangedRanges((r, i) => n = xt(n, r, i)),
      n = n.map(t.changes);
    let e = [];
    for (let r of t.effects) {
      r.is(U) && !Ie(n, r.value.from, r.value.to)
        ? e.push(r.value)
        : r.is(C) && (n = n.update({
    ...
    if (e.length) {
      let { preparePlaceholder: r } = t.state.facet(ct),
        i = e.map((s) =>
          (r ? p.replace({ widget: new nt(r(t.state, s)) }) : Tt).range(
            s.from,
            s.to,
          )
        );
    ...
    return t.selection && (n = xt(n, t.selection.main.head)), n;
  },
...
function wn(n) {
  return n.field(T, !1) || Lt.empty;
}
function R(n, t, e) {
  var r;
  let i = null;
  return (r = n.field(T, !1)) === null || r === void 0 ||
    r.between(t, e, (s, o) => {
...
function ut(n, t) {
  return n.field(T, !1) ? t : t.concat(K.appendConfig.of(Jt()));
}
var De = (n) => {
    for (let t of ft(n)) {
      let e = L(n.state, t.from, t.to);
      if (e) {
        return n.dispatch({ effects: ut(n.state, [U.of(e), z(n, e)]) }), !0;
      }
    }
    return !1;
  },
  Oe = (n) => {
    ...
    let t = [];
    for (let e of ft(n)) {
      let r = R(n.state, e.from, e.to);
      r && t.push(C.of(r), z(n, r, !1));
    }
    return t.length && n.dispatch({ effects: t }), t.length > 0;
  };
function z(n, t, e = !0) {
  let r = n.state.doc.lineAt(t.from).number,
    i = n.state.doc.lineAt(t.to).number;
  return b.announce.of(
    `${n.state.phrase(e ? "Folded lines" : "Unfolded lines")} ${r} ${
      n.state.phrase("to")
    } ${i}.`,
  );
}
var Be = (n) => {
    ...
    for (let r = 0; r < t.doc.length;) {
      let i = n.lineBlockAt(r), s = L(t, i.from, i.to);
      s && e.push(U.of(s)), r = (s ? n.lineBlockAt(s.to) : i).to + 1;
    }
    return e.length && n.dispatch({ effects: ut(n.state, e) }), !!e.length;
  },
  Me = (n) => {
    let t = n.state.field(T, !1);
    if (!t || !t.size) return !1;
    let e = [];
    return t.between(0, n.state.doc.length, (r, i) => {
      e.push(C.of({ from: r, to: i }));
    }),
      n.dispatch({ effects: e }),
      !0;
  };
function Ne(n, t) {
  for (let e = t;;) {
...
var yn = (n) => {
    ...
    for (let e of ft(n)) {
      let r = R(n.state, e.from, e.to);
      if (r) t.push(C.of(r), z(n, r, !1));
      else {
        let i = Ne(n, e);
        i && t.push(U.of(i), z(n, i));
      }
    }
    return t.length > 0 && n.dispatch({ effects: ut(n.state, t) }), !!t.length;
  },
  vn = [
    { key: "Ctrl-Shift-[", mac: "Cmd-Alt-[", run: De },
    { key: "Ctrl-Shift-]", mac: "Cmd-Alt-]", run: Oe },
    { key: "Ctrl-Alt-[", run: Be },
    { key: "Ctrl-Alt-]", run: Me },
  ],
  Le = {
...
function Jt(n) {
  let t = [T, Ee];
  return n && t.push(ct.of(n)), t;
}
function _t(n, t) {
  let { state: e } = n,
    r = e.facet(ct),
    i = (o) => {
      let a = n.lineBlockAt(n.posAtDOM(o.target)), l = R(n.state, a.from, a.to);
      l && n.dispatch({ effects: C.of(l) }), o.preventDefault();
    };
  if (r.placeholderDOM) return r.placeholderDOM(n, i, t);
...
var Tt = p.replace({
  ...
  O = class extends fe {
    ...
    toDOM(t) {
      ...
      return e.textContent = this.open
...
function xn(n = {}) {
  let t = { ...Re, ...n },
    ...
    i = F.fromClass(
      class {
        ...
        update(o) {
          (o.docChanged || o.viewportChanged ||
            o.startState.facet(x) != o.state.facet(x) ||
            o.startState.field(T, !1) != o.state.field(T, !1) ||
            g(o.startState) != g(o.state) || t.foldingChanged(o)) &&
            (this.markers = this.buildMarkers(o.view));
        }
        buildMarkers(o) {
          let a = new ht();
          for (let l of o.viewportLineBlocks) {
            let h = R(o.state, l.from, l.to)
              ? r
              : L(o.state, l.from, l.to)
              ? e
              : null;
  ...
  return [
    ...
    he({
      ...
      markers(o) {
        var a;
        return ((a = o.plugin(i)) === null || a === void 0
          ? void 0
          : a.markers) || Lt.empty;
      },
      initialSpacer() {
      ...
      domEventHandlers: {
        ...
        click: (o, a, l) => {
          ...
          let h = R(o.state, a.from, a.to);
          if (h) return o.dispatch({ effects: C.of(h) }), !0;
          let f = L(o.state, a.from, a.to);
          return f ? (o.dispatch({ effects: U.of(f) }), !0) : !1;
        },
      },
    }),
    Jt(),
...
var Ee = b.baseTheme({
  ...
  q = class n {
    constructor(t, e) {
      ...
      let s = typeof e.all == "string" ? e.all : e.all ? i(e.all) : void 0,
        o = e.scope;
      this.scope = o instanceof d
        ? (a) => a.prop(S) == o.data
        : o
        ? (a) => a == o
        : void 0,
...
function V(n) {
  let t = n.facet(rt);
  return t.length ? t : n.facet(Kt);
}
function Tn(n, t) {
  let e = [Fe], r;
  return n instanceof q &&
    (n.module && e.push(b.styleModule.of(n.module)), r = n.themeType),
    t?.fallback
      ? e.push(Kt.of(n))
      : r
      ? e.push(rt.computeN([b.darkTheme], (i) =>
        i.facet(b.darkTheme) == (r == "dark") ? [n] : []))
      : e.push(rt.of(n)),
    e;
}
function Sn(n, t, e) {
...
var it = class {
    ...
    update(t) {
      ...
      e.length < s.to && !i && e.type == this.tree.type && o >= s.to
        ? (this.decorations = this.decorations.map(t.changes),
          this.decoratedTo = o)
        : (e != this.tree || t.viewportChanged || i) &&
          (this.tree = e,
            this.decorations = this.buildDeco(t.view, r),
            this.decoratedTo = s.to);
    }
    buildDeco(t, e) {
      if (!e || !this.tree.length) return p.none;
      let r = new ht();
        ...
        ce(
          ...
          (o, a, l) => {
            r.add(
              o,
              a,
              this.markCache[l] || (this.markCache[l] = p.mark({ class: l })),
            );
          },
          i,
          s,
  ...
  Fe = Rt.high(F.fromClass(it, { decorations: (n) => n.decorations })),
  Pn = q.define([
  ...
  We = p.mark({ class: "cm-matchingBracket" }),
  He = p.mark({ class: "cm-nonmatchingBracket" });
function Ve(n) {
  let t = [], e = n.matched ? We : He;
  return t.push(e.range(n.start.from, n.start.to)),
    n.end && t.push(e.range(n.end.from, n.end.to)),
    t;
}
function St(n) {
  let t = [], e = n.facet(Yt);
  for (let r of n.selection.ranges) {
    if (!r.empty) continue;
    let i = W(n, r.head, -1, e) || r.head > 0 && W(n, r.head - 1, 1, e) ||
      e.afterCursor &&
        (W(n, r.head, 1, e) ||
          r.head < n.doc.length && W(n, r.head + 1, -1, e));
...
var $e = F.fromClass(
    class {
      constructor(n) {
        this.paused = !1, this.decorations = St(n.state);
      }
      update(n) {
        (n.docChanged || n.selectionSet || this.paused) &&
          (n.view.composing
            ? (this.decorations = this.decorations.map(n.changes),
              this.paused = !0)
            : (this.decorations = St(n.state), this.paused = !1));
      }
    },
...
function st(n, t, e) {
  ...
  if (n.name.length == 1) {
    ...
    if (i > -1 && i % 2 == (t < 0 ? 1 : 0)) return [e[i + t]];
  }
  return null;
}
function ot(n) {
  let t = n.type.prop(ze);
  return t ? t(n.node) : n;
}
function W(n, t, e, r = {}) {
  ...
  for (let l = a; l; l = l.parent) {
    ...
    if (h && l.from < l.to) {
      ...
      if (f && (e > 0 ? t >= f.from && t < f.to : t > f.from && t <= f.to)) {
        return qe(n, t, e, l, f, h, s);
      }
    }
  }
  return Ge(n, t, e, o, a.type, i, s);
}
function qe(n, t, e, r, i, s, o) {
  let a = r.parent, l = { from: i.from, to: i.to }, h = 0, f = a?.cursor();
  if (f && (e < 0 ? f.childBefore(r.from) : f.childAfter(r.to))) {
    do if (
      e < 0 ? f.to <= r.from : f.from >= r.to
    ) {
      ...
      } else if (st(f.type, e, o)) h++;
      else if (st(f.type, -e, o)) {
        if (h == 0) {
          let c = ot(f);
          return {
            start: l,
...
function Ge(n, t, e, r, i, s, o) {
  ...
  for (let w = 0; !f.next().done && w <= s;) {
    ...
    let P = t + w * e;
    for (
      let A = e > 0 ? 0 : k.length - 1, se = e > 0 ? k.length : -1;
      A != se;
      A += e
    ) {
      let Q = o.indexOf(k[A]);
      if (!(Q < 0 || r.resolveInner(P + A, 1).type != i)) {
        if (Q % 2 == 0 == e > 0) {
          c++;
        } else {
          if (c == 1) {
...
function Pt(n, t, e, r = 0, i = 0) {
  t == null && (t = n.search(/[^\s\u00a0]/), t == -1 && (t = n.length));
  let s = i;
  for (let o = r; o < t; o++) n.charCodeAt(o) == 9 ? s += e - s % e : s++;
  return s;
}
var G = class {
  constructor(t, e, r, i) {
  ...
  eat(t) {
    ...
    if (
      typeof t == "string"
        ? r = e == t
        : r = e && (t instanceof RegExp ? t.test(e) : t(e)), r
    ) return ++this.pos, e;
  }
  eatWhile(t) {
    let e = this.pos;
...
function Je(n) {
  return {
    ...
    copyState: n.copyState || _e,
    indent: n.indent || (() => null),
    languageData: n.languageData || {},
    tokenTable: n.tokenTable || pt,
...
var At = new WeakMap(),
  Ct = class n extends d {
    constructor(t) {
      let e = Ft(t.languageData),
        r = Je(t),
        i,
        s = new class extends Bt {
          createParse(o, a, l) {
            return new lt(i, o, a, l);
          }
        }();
      super(e, s, [], t.name),
        this.topNode = Ye(e, this),
        i = this,
        this.streamParser = r,
        this.stateAfter = new y({ perNode: !0 }),
        this.tokenTable = t.tokenTable ? new J(r.tokenTable) : Xe;
    }
    static define(t) {
      return new n(t);
    }
    getIndent(t) {
      ...
      let i = dt(this, t.node.tree, t.node.from, t.node.from, e ?? t.pos), s, o;
      if (
        i
          ? (o = i.state, s = i.pos + 1)
          : (o = this.streamParser.startState(t.unit), s = t.node.from),
          t.pos - s > 1e4
      ) return null;
      for (; s < t.pos;) {
        ...
        if (l.length) {
          let f = r ? r(l.from) : -1,
            c = new G(l.text, t.state.tabSize, t.unit, f < 0 ? void 0 : f);
          for (; c.pos < h - l.from;) te(this.streamParser.token, c, o);
        } else this.streamParser.blankLine(o, t.unit);
        if (h == t.pos) break;
        s = l.to + 1;
      }
      let a = t.lineAt(t.pos);
      return r && e == null && At.set(t.state, a.from),
        this.streamParser.indent(o, /^\s*(.*)/.exec(a.text)[1], t);
    }
    get allowsNesting() {
      return !1;
    }
...
function dt(n, t, e, r, i) {
  ...
  if (s) return { state: n.streamParser.copyState(s), pos: e + t.length };
  for (let o = t.children.length - 1; o >= 0; o--) {
    let a = t.children[o],
      l = e + t.positions[o],
      h = a instanceof m && l < i && dt(n, a, l, r, i);
    if (h) return h;
  }
  return null;
}
function Zt(n, t, e, r, i) {
  if (i && e <= 0 && r >= t.length) return t;
  !i && e == 0 && t.type == n.topNode && (i = !0);
  for (let s = t.children.length - 1; s >= 0; s--) {
    let o = t.positions[s], a = t.children[s], l;
    if (o < r && a instanceof m) {
      if (!(l = Zt(n, a, e - o, r - o, i))) break;
      return i
        ? new m(
          t.type,
          t.children.slice(0, s).concat(l),
...
function Ke(n, t, e, r, i) {
  for (let s of t) {
    let o = s.from + (s.openStart ? 25 : 0),
      a = s.to - (s.openEnd ? 25 : 0),
      l = o <= e && a > e && dt(n, s.tree, 0 - s.offset, e, a),
      h;
    if (
      l && l.pos <= r && (h = Zt(n, s.tree, e + s.offset, l.pos + s.offset, !1))
    ) return { state: l.state, tree: h };
  }
  return { state: n.streamParser.startState(i ? j(i) : 4), tree: m.empty };
}
var lt = class {
  constructor(t, e, r, i) {
    this.lang = t,
    ...
    let s = B.get(),
      o = i[0].from,
      { state: a, tree: l } = Ke(t, r, o, this.to, s?.state);
    ...
    s && this.parsedPos < s.viewport.from - 1e5 && i.some((h) =>
      h.from <= s.viewport.from && h.to >= s.viewport.from
    ) &&
    (this.state = this.lang.streamParser.startState(j(s.state)),
      s.skipUntilInView(this.parsedPos, s.viewport.from),
      this.parsedPos = s.viewport.from), this.moveRangeIndex();
  }
  advance() {
    let t = B.get(),
      e = this.stoppedAt == null ? this.to : Math.min(this.to, this.stoppedAt),
      r = Math.min(e, this.chunkStart + 512);
...
`);
  ...
  nextLine() {
    ...
    for (let i = this.rangeIndex;;) {
      ...
      if (
        s >= r ||
        (e = e.slice(0, s - (r - e.length)), i++, i == this.ranges.length)
      ) break;
  ...
  emitToken(t, e, r, i) {
    ...
    return this.lang.streamParser.mergeTokens && s == 4 && o >= 0 &&
        this.chunk[o] == t && this.chunk[o + 2] == e
      ? this.chunk[o + 2] = r
      : this.chunk.push(t, e, r, s),
      i;
  }
  parseLine(t) {
    let { line: e, end: r } = this.nextLine(),
      i = 0,
      { streamParser: s } = this.lang,
      o = new G(e, t ? t.state.tabSize : 4, t ? j(t.state) : 2);
    if (o.eol()) s.blankLine(this.state, o.indentUnit);
    else {for (; !o.eol();) {
        let a = te(s.token, o, this.state);
        if (
          a &&
          (i = this.emitToken(
  ...
  finishChunk() {
    ...
    t = new m(t.type, t.children, t.positions, t.length, [[
...
var J = class {
    constructor(t) {
      this.extra = t, this.table = Object.assign(Object.create(null), ee);
    }
    resolve(t) {
      return t ? this.table[t] || (this.table[t] = ne(this.extra, t)) : 0;
    }
  },
  Xe = new J(pt);
function Z(n, t) {
...
function ne(n, t) {
  ...
  for (let a of t.split(" ")) {
    ...
    for (let h of a.split(".")) {
      ...
      f
        ? typeof f == "function"
          ? l.length ? l = l.map(f) : Z(h, `Modifier ${h} used at start of tag`)
          : l.length
          ? Z(h, `Tag ${h} used as modifier`)
          : l = Array.isArray(f) ? f : [f]
        : Z(h, `Unknown highlighting tag ${h}`);
    }
  ...
  if (s) return s.id;
  let o = Dt[i] = _.define({ id: E.length, name: r, props: [de({ [r]: e })] });
...
function Ye(n, t) {
  let e = _.define({
    id: E.length,
    name: "Document",
    props: [S.add(() => n), jt.add(() => (r) => t.getIndent(r))],
    top: !0,
  });
  return E.push(e), e;
}
function re(n) {
  return n.length <= 4096 &&
    /[\u0590-\u05f4\u0600-\u06ff\u0700-\u08ac\ufb50-\ufdff]/.test(n);
}
function ie(n) {
  for (let t = n.iter(); !t.next().done;) if (re(t.value)) return !0;
  return !1;
}
function Ze(n) {
  let t = !1;
...
var at = v.define({ combine: (n) => n.some((t) => t) });
function Cn(n = {}) {
  let t = [tn];
  return n.alwaysIsolate && t.push(at.of(!0)), t;
}
var tn = F.fromClass(
  class {
    constructor(n) {
      this.always = n.state.facet(at) || n.textDirection != $.LTR ||
        n.state.facet(b.perLineTextDirection),
        this.hasRTL = !this.always && ie(n.state.doc),
        this.tree = g(n.state),
        this.decorations = this.always || this.hasRTL
          ? Ot(n, this.tree, this.always)
          : p.none;
    }
    update(n) {
      let t = n.state.facet(at) || n.view.textDirection != $.LTR ||
        n.state.facet(b.perLineTextDirection);
      if (
        !t && !this.hasRTL && Ze(n.changes) && (this.hasRTL = !0),
          !t && !this.hasRTL
      ) return;
      let e = g(n.state);
      (t != this.always || e != this.tree || n.docChanged ||
        n.viewportChanged) &&
        (this.tree = e, this.always = t, this.decorations = Ot(n.view, e, t));
    }
  },
  {
    provide: (n) => {
      function t(e) {
        var r, i;
        return (i = (r = e.plugin(n)) === null || r === void 0
                ? void 0
                : r.decorations) !== null && i !== void 0
          ? i
          : p.none;
      }
      return [b.outerDecorations.of(t), Rt.lowest(b.bidiIsolatedRanges.of(t))];
    },
  },
);
function Ot(n, t, e) {
  let r = new ht(), i = n.visibleRanges;
  e || (i = en(i, n.state.doc));
  for (let { from: s, to: o } of i) {
    t.iterate({
      enter: (a) => {
        let l = a.type.prop(y.isolate);
        l && r.add(a.from, a.to, nn[l]);
      },
      from: s,
      to: o,
    });
...
function en(n, t) {
  ...
  for (let { from: o, to: a } of n) {
    if (!(s && s.to > o && (o = s.to, o >= a))) {
      for (
        r + e.value.length < o && (e.next(o - (r + e.value.length)), r = o);;
      ) {
        let l = r, h = r + e.value.length;
        if (
          !e.lineBreak && re(e.value) &&
          (s && s.to > l - 10
            ? s.to = Math.min(a, h)
            : i.push(s = { from: l, to: Math.min(a, h) })), h >= a
        ) break;
        r = h, e.next();
      }
    }
...
export {
  ...
  C as unfoldEffect,
  Cn as bidiIsolates,
  Ct as StreamLanguage,
  d as Language,
  De as foldCode,
  dn as indentRange,
  et as TreeIndentContext,
  fn as forceParsing,
  Ft as defineLanguageFacet,
  G as StringStream,
  g as syntaxTree,
  ge as indentService,
  gn as continuedIndent,
  hn as syntaxTreeAvailable,
  Ht as indentUnit,
  j as getIndentUnit,
  Jt as codeFolding,
  jt as indentNodeProp,
  kn as indentOnInput,
  L as foldable,
  Me as unfoldAll,
  me as ensureSyntaxTree,
  mn as flatIndent,
  N as IndentContext,
  Oe as unfoldCode,
  Pe as foldNodeProp,
  pe as sublanguageProp,
  Pn as defaultHighlightStyle,
  pn as delimitedIndent,
  q as HighlightStyle,
  S as languageDataProp,
  Se as foldService,
  Sn as highlightingFor,
  T as foldState,
  Tn as syntaxHighlighting,
  tt as DocInput,
  U as foldEffect,
  un as syntaxParserRunning,
  vn as foldKeymap,
  Vt as indentString,
  vt as LanguageDescription,
  W as matchBrackets,
  wn as foldedRanges,
  x as language,
  xn as foldGutter,
  yn as toggleFold,
  yt as LanguageSupport,
  ze as bracketMatchingHandle,
};
//# sourceMappingURL=language.bundle.mjs.map
