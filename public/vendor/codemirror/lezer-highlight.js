...
import { NodeProp as V } from "@lezer/common";
var G = 0,
  p = class s {
    constructor(t, a, n, i) {
      this.name = t,
    ...
    toString() {
      let { name: t } = this;
      for (let a of this.modified) a.name && (t = `${a.name}(${t})`);
      return t;
    }
    static define(t, a) {
      let n = typeof t == "string" ? t : "?";
      if (t instanceof s && (a = t), a?.base) {
        throw new Error("Can not derive from a modified tag");
      }
      let i = new s(n, [], null, []);
      if (i.set.push(i), a) { for (let o of a.set) i.set.push(o); }
      return i;
    }
    static defineModifier(t) {
      let a = new E(t);
      return (n) =>
        n.modified.indexOf(a) > -1 ? n : E.get(
          n.base || n,
          n.modified.concat(a).sort((i, o) => i.id - o.id),
        );
    }
  },
  L = 0,
  E = class s {
    constructor(t) {
      this.name = t, this.instances = [], this.id = L++;
    }
    static get(t, a) {
      if (!a.length) return t;
      let n = a[0].instances.find((r) => r.base == t && P(a, r.modified));
      if (n) return n;
      let i = [], o = new p(t.name, i, t, a);
      for (let r of a) r.instances.push(o);
      let h = Q(a);
      for (let r of t.set) {
        if (!r.modified.length) { for (let d of h) i.push(s.get(r, d)); }
      }
      return o;
    }
  };
...
function Q(s) {
  ...
  for (let a = 0; a < s.length; a++) {
    for (let n = 0, i = t.length; n < i; n++) t.push(t[n].concat(s[a]));
  }
    ...
    for (let i of a.split(" ")) {
      if (i) {
        ...
        for (let g = 0;;) {
          ...
          if (
            o.push(f[0] == "*" ? "" : f[0][0] == '"' ? JSON.parse(f[0]) : f[0]),
              g += f[0].length,
              g == i.length
          ) break;
          let m = i[g++];
          ...
          if (m != "/") throw new RangeError("Invalid path: " + i);
          r = i.slice(g);
        }
        let d = o.length - 1, c = o[d];
        if (!c) throw new RangeError("Invalid path: " + i);
        let x = new v(n, h, d > 0 ? o.slice(0, d) : null);
        t[c] = x.sort(t[c]);
      }
    }
  }
      ...
      for (; s || t;) {
        if (
          !s || t && s.depth >= t.depth
            ? (i = t, t = t.next)
            : (i = s, s = s.next),
            a && a.mode == i.mode && !i.context && !a.context
        ) continue;
        let o = new v(i.tags, i.mode, i.context);
        a ? a.next = o : n = o, a = o;
      }
      return n;
    },
...
function U(s, t) {
  ...
  for (let o of s) {
    ...
    else for (let h of o.tag) a[h.id] = o.class;
  }
...
function X(s, t, a, n = 0, i = s.length) {
  let o = new B(n, Array.isArray(t) ? t : [t], a);
  o.highlightRange(s.cursor(), n, i, "", o.highlighters), o.flush(i);
}
function tt(s, t, a, n, i, o = 0, h = s.length) {
  let r = o;
  function d(c, x) {
    if (!(c <= r)) {
      for (let g = s.slice(r, c), f = 0;;) {
        let m = g.indexOf(
            `
`,
            f,
          ...
          R = m < 0 ? g.length : m;
        if (R > f && n(g.slice(f, R), x), m < 0) break;
        i(), f = m + 1;
      }
      r = c;
    }
...
var B = class {
  ...
  startSpan(t, a) {
    a != this.class &&
      (this.flush(t), t > this.at && (this.at = t), this.class = a);
  }
  flush(t) {
    t > this.at && this.class && this.span(this.at, t, this.class);
  }
  highlightRange(t, a, n, i, o) {
    let { type: h, from: r, to: d } = t;
    if (r >= n || d <= a) return;
    h.isTop && (o = this.highlighters.filter((m) => !m.scope || m.scope(h)));
    let c = i, x = Y(t) || v.empty, g = W(o, x.tags);
    if (
      g && (c && (c += " "), c += g, x.mode == 1 && (i += (i ? " " : "") + g)),
        this.startSpan(Math.max(a, r), c),
        x.opaque
    ) return;
    let f = t.tree && t.tree.prop(V.mounted);
    if (f && f.overlay) {
      let m = t.node.enter(f.overlay[0].from + r, 1),
        R = this.highlighters.filter((M) => !M.scope || M.scope(f.tree.type)),
        H = t.firstChild();
      for (let M = 0, S = r;; M++) {
        let O = M < f.overlay.length ? f.overlay[M] : null,
          K = O ? O.from + r : d,
          $ = Math.max(a, S),
          T = Math.min(n, K);
        if ($ < T && H) {
          for (
            ;
            t.from < T &&
            (this.highlightRange(t, $, T, i, o),
              this.startSpan(Math.min(T, t.to), c),
              !(t.to >= K || !t.nextSibling()));
          );
        }
        if (!O || K > n) break;
        S = O.to + r,
      ...
      do if (!(t.to <= a)) {
        if (t.from >= n) break;
        this.highlightRange(t, a, n, i, o),
          this.startSpan(Math.min(n, t.to), c);
      } while (t.nextSibling());
      t.parent();
    }
  }
...
var et = U([
  ...
  {
    tag: l.definition(l.variableName),
    class: "tok-variableName tok-definition",
  },
  { tag: l.special(l.variableName), class: "tok-variableName2" },
  {
    tag: l.definition(l.propertyName),
...
//# sourceMappingURL=highlight.bundle.mjs.map
